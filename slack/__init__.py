...
def slack_source(
    ...
    logger.info("Available channels: %s", [c["name"] for c in channels])
    logger.info("Selected channels: %s", [c["name"] for c in fetched_selected_channels])
    ...
    def get_thread_replies(messages: List[Dict[str, Any]]) -> Iterable[TDataItem]:
        for message in messages:
            if message.get("thread_ts"):
                logger.info(f">>>> fetching replies for thread_ts={message['thread_ts']} in channel={message['channel']}")
                params = {
                    "channel": message["channel"],
                    "ts": ensure_dt_type(message["thread_ts"], to_ts=True),
                }
                ...
                for page_data in api.get_pages(
                    ...
                    yield from page_data[1:]  # skip root message
    ...
    if table_per_channel:
        ...
        for channel in fetched_selected_channels:
            ...
            if replies:
                ...
                #     write_disposition=write_disposition,
                # )
                replies_resource = dlt.transformer(
                    get_thread_replies,
                    name=channel_name + "_replies",
                    table_name=partial(table_name_func, channel_name + "_replies"),
                    primary_key=("thread_ts", "ts"),
                ...
                yield messages_channel | replies_resource
